...
async function testFinalFormat() {
  try {
    console.log("Testing final PDF generation with all formatting updates...");
...
3. Begin project development`;
    ...
    // Generate PDF
    const pdfBuffer = await generatePDF(sampleContent, "final-format-test");

    // Save to file
    fs.writeFileSync("final-format-test.pdf", Buffer.from(pdfBuffer));

    console.log("✅ PDF generated successfully! Check final-format-test.pdf");
