...
async function testCompanyHeader() {
  try {
    console.log("Testing PDF generation with company header...");
...
3. Begin project development`;
    ...
    // Generate PDF
    const pdfBuffer = await generatePDF(sampleContent, "company-header-test");

    // Save to file
    fs.writeFileSync("company-header-test.pdf", Buffer.from(pdfBuffer));

    console.log("✅ PDF generated successfully! Check company-header-test.pdf");
    console.log("📋 The PDF should have:");
    console.log(
      '   - Company name "TRIOSTACK TECHNOLOGIES PRIVATE LIMITED" at the top'
    );
    console.log("   - Company details with bold labels: CIN | Phone | Address");
    console.log("   - Smaller, beautified fonts (9pt)");
