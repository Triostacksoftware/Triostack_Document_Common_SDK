...
async function testUltimateFix() {
  try {
    console.log("Testing ULTIMATE fix for address clipping and centering...");
...
3. Begin project development`;
    ...
    // Generate PDF
    const pdfBuffer = await generatePDF(sampleContent, "ultimate-fix-test");

    // Save to file
    fs.writeFileSync("ultimate-fix-test.pdf", Buffer.from(pdfBuffer));

    console.log("✅ PDF generated successfully! Check ultimate-fix-test.pdf");
    console.log("📋 The PDF should have:");
    console.log(
      '   - Company name "TRIOSTACK TECHNOLOGIES PRIVATE LIMITED" at the top (centered)'
    );
    console.log("   - Company details with ULTIMATE fix:");
    console.log("     * Line 1: CIN and Phone (centered)");
    console.log("     * Line 2: Address (centered, with smaller max width)");
    console.log(
