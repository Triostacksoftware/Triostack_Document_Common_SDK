...
async function testAddressWrap() {
  try {
    console.log("Testing PDF generation with address wrapping...");
...
3. Begin project development`;
    ...
    // Generate PDF
    const pdfBuffer = await generatePDF(sampleContent, "address-wrap-test");

    // Save to file
    fs.writeFileSync("address-wrap-test.pdf", Buffer.from(pdfBuffer));

    console.log("✅ PDF generated successfully! Check address-wrap-test.pdf");
